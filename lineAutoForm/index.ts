  ...
  try {
    ...
    if (eventtype == 'follow') {
      ...
      if (responseNo > 0) {
        ...
        // リッチメニューを選んだら再送
        if (richMessageArray.includes(messageStr)) {
          // レスポンスNOを戻す
          responseNo--;
          // キャッシュ
          const obj: any = {
            no: responseNo, // キャッシュNO
            message: '', // メッセージ
          };
          // キャッシュ保存
          ...
          dataString = JSON.stringify({
            ...
            messages: [
              {
                type: 'text', // テキスト
                text: messageArray[responseNo], // メッセージ
              },
            ],
          });
          // wait for time
          ...
          if (messageStr == resetStr) {
            ...
            const lineuserValues: any[] = [
              userId, // ユーザID
              cache1.message, // ユーザ名
              cache2.message, // ユーザ電話番号
              cache3.message, // ユーザ郵便番号
              cache4.message, // ユーザ住所
              messageStr, // ユーザラベル文字列
              1, // 完了
              1, // 利用可能
            ];
            ...
            const insertLineuserArgs: insertargs = {
              table: 'lineuser', // テーブル
              columns: lineuserColumns, // 配信カラム
              values: lineuserValues, // 配信値
            }
            // DB挿入
